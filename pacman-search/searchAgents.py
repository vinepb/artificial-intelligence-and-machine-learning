            ...
            if not hitsWall:
                ...
                for i in state[1]:
                    if not i == (nextx, nexty):
                        nonvisited.append(i)
                successors.append(
                    (((nextx, nexty), tuple(nonvisited)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for i in nonvisited:
        d = abs(i[0] - pos[0]) + abs(i[1] - pos[1])  # Manhattan (better)
        # d = (((i[0] - pos[0]) ** 2) + ((i[1] - pos[1]) ** 2)) ** 0.5 # Euclidean
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    heuristic = 0
    nonEatenFoodList = foodGrid.asList()
    foodDistances = []

    if len(nonEatenFoodList) == 0:
        return heuristic

    for i in nonEatenFoodList:
        for j in nonEatenFoodList:
            d = mazeDistance(i, j, problem.startingGameState)
            foodDistances.append((d, i, j))
    foodDistances = sorted(foodDistances, reverse=True)

    heuristic += foodDistances[0][0]
    heuristic += min(mazeDistance(foodDistances[0][1], position, problem.startingGameState),
                     mazeDistance(foodDistances[0][2], position, problem.startingGameState))
